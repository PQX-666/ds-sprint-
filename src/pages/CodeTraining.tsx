...
export default function CodeTraining() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-text">核心代码训练</h1>
        ...
        {codeTrainingItems.map((item) => (
          <button
            ...
            className={`text-sm px-3 py-1.5 rounded-lg border cursor-pointer transition-colors ${
              activeId === item.id ? 'bg-primary text-white border-primary' : 'bg-surface text-text border-border hover:border-primary/50'
            }`}
          >
            {item.title}
      ...
      {active && (
        <div className="bg-surface rounded-xl border border-border p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                <span className="text-xs text-primary bg-primary/10 px-2 py-0.5 rounded">{active.examFocus}</span>
              </div>
            </div>
          ...
          <div>
            <p className="text-sm font-medium text-text mb-2">完整代码：</p>
            ...
            <ol className="space-y-1">
              {active.lineByLineExplanation.map((line, i) => (
                <li key={i} className="text-sm text-text-muted flex gap-2">
                  <span className="text-primary shrink-0">{i + 1}.</span> {line}
                </li>
              ))}
            </ol>
          </div>
          ...
          <div>
            ...
            <pre className="bg-gray-900 text-gray-100 rounded-lg p-4 overflow-x-auto text-sm leading-relaxed font-mono whitespace-pre-wrap">
                ...
                <span key={i}>
                  ...
                  {i < active.fillBlank.template.split('________').length - 1 && (
                    <input
                      ...
                      className="inline-block bg-gray-700 text-yellow-300 border-b border-yellow-500 outline-none px-1 mx-0.5 font-mono text-sm"
            ...
            {!fillChecked && (
              <button
                ...
                className="mt-2 text-sm text-primary hover:text-primary-dark cursor-pointer"
              >
                核对填空答案
              </button>
            )}
            {fillChecked && (
              <div className="mt-2 p-3 bg-callout-success-bg border border-callout-success-border rounded-lg">
                ...
                <button onClick={() => { setFillChecked(false); setFillInputs({}); }} className="mt-2 text-xs text-primary cursor-pointer">重填</button>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={() => { markMastered(active.id); handleActivate(active.id); }} className="bg-success text-white px-4 py-2 rounded-lg text-sm font-medium cursor-pointer hover:bg-success/80">我做对了</button>
            <button onClick={() => markFuzzy(active.id)} className="bg-accent text-white px-4 py-2 rounded-lg text-sm font-medium cursor-pointer hover:bg-accent/80">有点模糊</button>
            <button onClick={() => {
              markWrong(active.id);
              addItem({ questionId: active.id, type: 'code', title: active.title, correctAnswer: active.answer, explanation: active.idea });
            }} className="bg-danger text-white px-4 py-2 rounded-lg text-sm font-medium cursor-pointer hover:bg-danger/80">做错了</button>
          </div>
        </div>
      )}
    </div>
